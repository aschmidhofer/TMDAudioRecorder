...
// Taken from https://www.programcreek.com/java-api-examples/index.php?source_dir=roman10-android-tutorial-master/AndroidWaveRecorder/src/roman10/tutorial/androidwaverecorder/WavAudioRecorder.java#
...
public class WavAudioRecorder {
    ...
    public static WavAudioRecorder getInstanse(int sampleRate) {
        WavAudioRecorder result = new WavAudioRecorder(AudioSource.MIC,
                    sampleRate,
    ...
    // time stamp when to stop recording ( or 0 if open-end)
    private long endtime = 0;
    private AService service; // service to stop when done

    private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
        // periodic updates on the progress of the record head
        public void onPeriodicNotification(AudioRecord recorder) {
            if (State.STOPPED == state) {
                Log.d(AnActivity.TAG, "recorder stopped");
                return;
            }
            if(endtime>0){
                if(System.currentTimeMillis()>endtime) {
                    stop();
                    Log.d(AnActivity.TAG, "recording stopped due to timer");
                    if (service!=null)
                        service.stopSelf();
                    else {
                        Log.e(AnActivity.TAG, "service to destroy not specified");
                    }
                    return;
                }
            }
...
//   Log.d(WavAudioRecorder.this.getClass().getName(), state + ":" + numOfBytes);
    ...
    public void setEndTime(long timestamp, AService service){
        this.service = service;
        endtime = timestamp;
        Log.d(AnActivity.TAG, "recording started with timer at "+timestamp);
    }
}